...
export function ResultsList({ results, selectedFuel, text, hasSearched, theme }) {
  if (!hasSearched) {
    return (
      <div className={theme === 'light' ? 'flex min-h-80 items-center justify-center rounded-lg border border-orange-300 bg-orange-50 p-6 text-center' : 'flex min-h-80 items-center justify-center rounded-lg border border-aqua/20 bg-[#0b0d0d] p-6 text-center'}>
        <div>
  ...
  if (!results.length) {
    return (
      <div className={theme === 'light' ? 'flex min-h-80 items-center justify-center rounded-lg border border-gray-300 bg-gray-100 p-6 text-center' : 'flex min-h-80 items-center justify-center rounded-lg border border-white/10 bg-zinc-950 p-6 text-center'}>
        <div>
          <p className={theme === 'light' ? 'text-base font-bold text-gray-900' : 'text-base font-bold text-white'}>{text.results.emptyTitle}</p>
          <p className={theme === 'light' ? 'mt-2 text-sm text-gray-700' : 'mt-2 text-sm text-zinc-400'}>{text.results.emptyHelp}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={theme === 'light' ? 'h-[70vh] overflow-y-auto rounded-lg border border-gray-300 bg-gray-50 p-2 sm:p-3' : 'h-[70vh] overflow-y-auto rounded-lg border border-white/10 bg-[#0b0d0d] p-2 sm:p-3'}>
      <div className="space-y-2.5">
        {results.map((station, index) => (
          <StationCard station={station} key={station.id} index={index} selectedFuel={selectedFuel} text={text} theme={theme} />
        ))}
      </div>
    </div>
  );
...
function StationCard({ station, index, selectedFuel, text, theme }) {
  ...
  return (
    <a
      className={theme === 'light' ? 'group block rounded-lg border border-orange-300 bg-white p-3 transition duration-200 hover:-translate-y-0.5 hover:border-orange-500 hover:shadow-orange-200 sm:p-4' : 'group block rounded-lg border border-white/10 bg-black/70 p-3 transition duration-200 hover:-translate-y-0.5 hover:border-ember/60 hover:shadow-ember sm:p-4'}
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className={theme === 'light' ? 'mt-2 flex gap-2 text-xs text-gray-700 sm:text-sm' : 'mt-2 flex gap-2 text-xs text-zinc-300 sm:text-sm'}>
            <MapPin size={15} className={theme === 'light' ? 'mt-0.5 shrink-0 text-orange-600' : 'mt-0.5 shrink-0 text-aqua'} />
            <span>
              <span className={theme === 'light' ? 'font-bold text-orange-600' : 'font-bold text-aqua'}>{text.results.location}: </span>
              {station.address}
              <span className={theme === 'light' ? 'block text-gray-600' : 'block text-zinc-500'}>{station.municipality || station.locality}</span>
            </span>
          </p>
        </div>
